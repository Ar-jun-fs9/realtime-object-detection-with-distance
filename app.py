...
FOCAL_LENGTH = 550  # Approximate focal length for 640x480 (adjust if needed)
...
# Optional: Text-to-speech function (commented out for server compatibility)
def speak(text):
    # Uncomment if you want TTS, but may fail on Railway
    """
    def _speak():
        import pyttsx3
        engine = pyttsx3.init()
...
def calculate_distance(class_name, pixel_height):
    ...
    if pixel_height > 0:
        return round((real_height * FOCAL_LENGTH) / pixel_height, 2)
    return None


# Process uploaded frame for detection
...
# Optional route for browser webcam frames
